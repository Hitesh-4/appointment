  ...
  const role = user?.publicMetadata?.role; 
    return (
      ...
      <div className="w-full  flex flex-col mt-10 mb-10 gap-3 relative rounded-xl border-t-2 border-x-2 border-[#9e9e9e1f] max-md:text-sm overflow-hidden">
        <div className="head grid grid-cols-9  max-md:grid-cols-5  w-full bg-zinc-950 h-14 items-center justify-between pl-10 max-md:px-1 max-lg:px-2">
          <p>Patient</p>
    ...
    <div>
      {Array.isArray(users) && users.length > 0 ? (
        users.map((item: any) => (
          <div
            key={item.id}
            className="grid grid-cols-9 max-md:grid-cols-5  overflow-x-auto text-xs items-center max-md:text-sm justify-between pl-10 max-md:px-1 max-lg:px-2 h-14 border-b-2 border-[#9e9e9e1f]"
          >
